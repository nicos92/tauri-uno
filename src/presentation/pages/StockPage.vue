...
const stockCompletos = computed(() => {
  return stockStore.stocks.map((s) => {
    const articulo = articulosStore.articulos.find((a) => a.id === s.id_articulo);
    const precioVenta = preciosVenta.value.get(s.id) || (s.costo * (1 + s.ganancia / 100));
    return {
      ...s,
...
const articulosDisponibles = computed(() => {
  return articulosStore.articulos.filter((a) => {
    const tieneStock = stockStore.stocks.some((s) => s.id_articulo === a.id);
    return !tieneStock;
  });
});
...
<template>
  <div class="stock-page">
    <div class="page-header">
      ...
      <button v-if="canCreateStock()" @click="openCreateModal" class="btn-primary">
        Crear Stock
      </button>
    </div>

    <div v-if="stockStore.loading || articulosStore.loading" class="loading">
      Cargando...
    </div>
    ...
    <table v-else class="stock-table">
      ...
      <tbody>
        <tr v-for="stock in stockCompletos" :key="stock.id">
          ...
          <td class="actions">
            <button v-if="canUpdateStock()" @click="openEditModal(stock)" class="btn-icon" title="Editar">
              <img src="/svg/edit.svg" alt="Editar" />
            </button>
            <button v-if="canDeleteStock()" @click="handleDelete(stock.id)" class="btn-icon btn-danger" title="Eliminar">
        ...
        <form @submit.prevent="handleCreate">
          <div class="form-group">
            ...
            <select v-model="newIdArticulo" required>
              ...
              <option v-for="art in articulosDisponibles" :key="art.id" :value="art.id">
          ...
          <div class="form-group">
            ...
            <input v-model.number="newCantidad" type="number" step="0.01" min="0" required />
          </div>
          <div class="form-group">
            <label>Costo</label>
            <input v-model.number="newCosto" type="number" step="0.01" min="0" required />
          </div>
          <div class="form-group">
            <label>Ganancia (%)</label>
            <input v-model.number="newGanancia" type="number" step="0.01" min="0" required />
          </div>
          ...
          <div class="modal-actions">
            ...
            <button type="submit" class="btn-primary" :disabled="!newIdArticulo">
              Crear
            </button>
          </div>
        </form>
    ...
    <div v-if="showEditModal" class="modal-overlay" @click.self="showEditModal = false">
      <div class="modal">
        ...
        <form @submit.prevent="handleUpdate">
          <div class="form-group">
            <label>Cantidad</label>
            <input v-model.number="editCantidad" type="number" step="0.01" min="0" required />
          </div>
          <div class="form-group">
            <label>Costo</label>
            <input v-model.number="editCosto" type="number" step="0.01" min="0" required />
          </div>
          <div class="form-group">
            <label>Ganancia (%)</label>
            <input v-model.number="editGanancia" type="number" step="0.01" min="0" required />
          </div>
          <div v-if="stockStore.error" class="error-message">
            {{ stockStore.error }}
          </div>
